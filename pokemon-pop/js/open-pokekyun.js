...
(function () {
  ...
  /** Gold hit (UR-style): Pikachu + Flareon EX #007 */
  const GOLD_HIT_IDS = { "cp3-010": 1, "cp3-007": 1 };
  ...
  const openFxSeam = document.getElementById("pokekyun-fx-seam-path");
  const openFxFanFill = document.getElementById("pokekyun-fx-fan-fill");
  const openFxFanRays = document.getElementById("pokekyun-fx-fan-rays");
  const openFxAbovePoly = document.getElementById("pokekyun-fx-above-poly");
  ...
  function isGoldHit(card) {
    return !!(card && GOLD_HIT_IDS[card.id]);
  }

  function isHit(card) {
    return !!(card && String(card.rarity || "").toUpperCase() === "RR");
  }

  function isFoil(card) {
    if (!card) return false;
    const rarity = String(card.rarity || "").toUpperCase();
    return !!FOIL_RARITIES[rarity];
  }
  ...
  function arHoloStyle(card) {
    if (!card) return "holo";
    if (isGoldHit(card)) return "holo";
    if (rarityKey(card) === "RR") return "holo";
    return PT.resolveHoloStyle ? PT.resolveHoloStyle(card) : card.holoStyle || "reverse";
  }
